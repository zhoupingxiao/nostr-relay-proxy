...
let refreshTimer;function schedule(seconds=60){clearTimeout(refreshTimer);refreshTimer=setTimeout(refresh,Math.max(60,Number(seconds)||60)*1000)}async function refresh(){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),4000);try{const r=await fetch('/status',{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error('status unavailable');const s=await r.json();const active=(s.relays||[]).filter(x=>x.connected).length;const dot=document.getElementById('dot');dot.className='dot '+(s.online?'on':'');document.getElementById('summary').textContent=s.degraded?ui('unavailable'):s.online?ui('serviceOnline')+' · '+fill('upstreamsConnected',active):ui('waitingUpstream');document.getElementById('updated').textContent=ui('readAt').replace('{time}',new Date().toLocaleTimeString());const metrics=document.getElementById('metrics');metrics.replaceChildren(metric('effectiveUsers',s.clients||0),metric('clientSubscriptions',s.clientSubscriptions??s.subscriptions??0),metric('upstreamSubscriptions',s.upstreamSubscriptions??0),metric('availableRelays',active+' / '+(s.relays||[]).length),metric('serviceUptime',ago(s.stats?.startedAt)));const list=document.getElementById('relays');list.replaceChildren(...(s.relays||[]).map(x=>{let row=document.createElement('div');row.className='relay';let left=document.createElement('div');let name=document.createElement('div');name.className='relay-name';name.textContent=x.url;let meta=document.createElement('div');meta.className='muted';meta.textContent=x.connected?ui('connected')+' '+ago(x.lastConnectedAt):x.enabled?ui('waitingReconnect').replace('{n}',x.reconnects):ui('disabled');left.append(name,meta);let badge=document.createElement('span');badge.className='badge '+(x.connected?'on':'off');badge.textContent=x.connected?ui('online'):ui('offline');row.append(left,badge);return row}));if(!s.degraded)schedule(s.settings?.statusRefreshSeconds)}catch{document.getElementById('summary').textContent=ui('unavailable')}finally{clearTimeout(timeout)}}

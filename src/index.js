...
const UI_TRANSLATIONS = {
  zh: {
    admin: "管理后台", viewHome: "查看前台", logout: "退出", copyUrl: "复制连接地址", copied: "已复制",
    serviceOnline: "服务在线", waitingUpstream: "正在等待上游 Relay 连接", upstreamsConnected: "{n} 个上游已连接", readAt: "读取于 {time}",
    upstreamRelay: "上游 Relay", upstreamDescription: "已连接的上游会自动同步订阅；离线节点会采用退避策略重连。", homeTitle1: "一个更可靠的", homeTitle2: " Nostr 连接入口。", homeDescription: "聚合多个上游 Relay，为你的客户端提供统一、稳定的 WebSocket 连接。上游 Relay 列表会按后台设定的安全间隔自动更新，兼顾状态及时性与 Cloudflare 免费额度。", loading: "加载中…",
    effectiveUsers: "有效用户", onlineUsers: "在线用户", clientSubscriptions: "客户端活跃订阅", upstreamSubscriptions: "上游活跃订阅", chineseLanguage: "中文", englishLanguage: "English",
    availableRelays: "可用 Relay", serviceUptime: "服务运行", connected: "已连接", waitingReconnect: "等待重连 · 已重连 {n} 次", disabled: "已由管理员停用",
    online: "在线", offline: "离线", justNow: "刚刚", hoursMinutes: "{h} 小时 {m} 分钟", minutesSeconds: "{m} 分 {s} 秒", seconds: "{s} 秒", unavailable: "暂时无法读取状态",
    welcome: "欢迎回来", loginDescription: "登录后管理上游 Relay 与实时网络状态。", loginTitle: "登录 · Nostr Relay Proxy", adminTitle: "管理后台 · Nostr Relay Proxy", username: "管理员用户名", password: "密码", safeLogin: "安全登录", loggingIn: "登录中…", loginFailed: "登录失败",
    ...
    accessControl: "访问控制", accessDescription: "白名单和黑名单会要求客户端完成 NIP-42 身份认证；普通事件和订阅受规则限制，私信 kind 4、13、14、1059 不受黑白名单限制；每行一条，格式为：公钥 | 用户名（用户名可选）。", all: "全部", whitelist: "白名单", blacklist: "黑名单", userPubkey: "公钥（npub 或 hex）", userName: "用户名", addUser: "新增用户", remove: "删除",
    ...
    savedSettings: "已保存中继资料、优先中继、刷新间隔和访问控制", saving: "正在保存…", saveFailed: "保存失败", requestFailed: "请求失败", unauthorized: "未授权", enable: "启用", disable: "停用", delete: "删除", confirmDelete: "删除 {url}？", disabledRelay: "（已停用）", noError: "—", noValue: "—", relayUp: "在线", relayDown: "离线",
    authenticatedUsers: "已认证用户", onlineConnections: "在线连接", availableUpstream: "可用上游", uploadedTraffic: "上传流量", downloadedTraffic: "下载流量", clientEvents: "客户端事件", dragHandle: "拖动排序", sortLatency: "点击按延迟排序", hintStorage: "提示：上游列表、中继资料和优先中继都保存在 Durable Object；Cloudflare 中的变量仅用于首次默认值。",
    invalidLogin: "用户名或密码错误", missingCredentials: "当前 Worker 未读取到 ADMIN_USER 或 ADMIN_PASSWORD，请在 Variables and Secrets 保存并部署到当前环境。"
  },
  en: {
    admin: "Admin", viewHome: "View home", logout: "Sign out", copyUrl: "Copy connection URL", copied: "Copied",
    serviceOnline: "Service online", waitingUpstream: "Waiting for upstream relays", upstreamsConnected: "{n} upstream(s) connected", readAt: "Read at {time}",
    upstreamRelay: "Upstream Relay", homeTitle1: "A more reliable", homeTitle2: " Nostr connection gateway.", homeDescription: "Aggregate multiple upstream Relays into one stable WebSocket entry point. The upstream list refreshes at the configured safety interval to balance freshness and Cloudflare quota.", upstreamDescription: "Connected upstreams sync subscriptions; offline nodes reconnect with backoff.", loading: "Loading…",
    effectiveUsers: "Effective users", onlineUsers: "Online users", clientSubscriptions: "Client active subscriptions", upstreamSubscriptions: "Upstream active subscriptions", chineseLanguage: "Chinese", englishLanguage: "English",
    availableRelays: "Available Relays", serviceUptime: "Service uptime", connected: "Connected", waitingReconnect: "Waiting to reconnect · {n} reconnects", disabled: "Disabled by administrator",
    online: "Online", offline: "Offline", justNow: "Just now", hoursMinutes: "{h} h {m} min", minutesSeconds: "{m} min {s} sec", seconds: "{s} sec", unavailable: "Unable to read status",
    welcome: "Welcome back", loginDescription: "Sign in to manage upstream Relays and network status.", loginTitle: "Sign in · Nostr Relay Proxy", adminTitle: "Admin · Nostr Relay Proxy", username: "Admin username", password: "Password", safeLogin: "Sign in securely", loggingIn: "Signing in…", loginFailed: "Login failed",
    ...
    accessControl: "Access control", accessDescription: "Whitelist and blacklist modes require NIP-42 authentication; normal events and subscriptions follow the rule, while private-message kinds 4, 13, 14 and 1059 bypass it; one entry per row: pubkey | username (username is optional).", all: "All", whitelist: "Whitelist", blacklist: "Blacklist", userPubkey: "Pubkey (npub or hex)", userName: "Username", addUser: "Add user", remove: "Remove",
    ...
    savedSettings: "Relay profile, priority Relay, refresh interval and access control saved", saving: "Saving…", saveFailed: "Save failed", requestFailed: "Request failed", unauthorized: "Unauthorized", enable: "Enable", disable: "Disable", delete: "Delete", confirmDelete: "Delete {url}?", disabledRelay: " (disabled)", noError: "—", noValue: "—", relayUp: "Online", relayDown: "Offline",
    authenticatedUsers: "Authenticated users", onlineConnections: "Online connections", availableUpstream: "Available upstream", uploadedTraffic: "Uploaded traffic", downloadedTraffic: "Downloaded traffic", clientEvents: "Client events", dragHandle: "Drag to reorder", sortLatency: "Click to sort by latency", hintStorage: "Hint: upstream Relays, relay profile and priority Relay are stored in the Durable Object; Cloudflare variables are only used as first-run defaults.",
    invalidLogin: "Invalid username or password", missingCredentials: "This Worker cannot read ADMIN_USER or ADMIN_PASSWORD. Save them under Variables and Secrets and deploy the current environment."
  }
};

const UI_STATIC_KEYS = {
  "管理后台": "admin", "查看前台": "viewHome", "退出": "logout", "复制连接地址": "copyUrl", "正在获取网络状态…": "loading", "一个更可靠的": "homeTitle1", "Nostr 连接入口。": "homeTitle2", "聚合多个上游 Relay，为你的客户端提供统一、稳定的 WebSocket 连接。上游 Relay 列表会按后台设定的安全间隔自动更新，兼顾状态及时性与 Cloudflare 免费额度。": "homeDescription", "中文": "chineseLanguage", "English": "englishLanguage",
  "上游 Relay": "upstreamRelay", "已连接的上游会自动同步订阅；离线节点会采用退避策略重连。": "upstreamDescription", "加载中…": "loading",
  ...
  "在线用户": "onlineUsers", "有效用户": "effectiveUsers", "客户端活跃订阅": "clientSubscriptions", "上游活跃订阅": "upstreamSubscriptions", "可用 Relay": "availableRelays", "服务运行": "serviceUptime", "已认证用户": "authenticatedUsers", "在线连接": "onlineConnections", "可用上游": "availableUpstream", "上传流量": "uploadedTraffic", "下载流量": "downloadedTraffic", "客户端事件": "clientEvents", "拖动排序": "dragHandle", "点击按延迟排序": "sortLatency", "在线": "online", "离线": "offline", "已停用": "disabled", "启用": "enable", "停用": "disable", "复制": "copy", "已复制": "copied", "删除": "delete", "请求失败": "requestFailed", "保存失败": "saveFailed", "未授权": "unauthorized", "用户名或密码错误": "invalidLogin", "当前 Worker 未读取到 ADMIN_USER 或 ADMIN_PASSWORD，请在 Variables and Secrets 保存并部署到当前环境。": "missingCredentials", "已保存中继资料、优先中继、刷新间隔和访问控制": "savedSettings", "正在保存…": "saving", "—": "noValue", "不设置（仅按延迟）": "noPriority", "提示：上游列表、中继资料和优先中继都保存在 Durable Object；Cloudflare 中的变量仅用于首次默认值。": "hintStorage", "白名单和黑名单会要求客户端完成 NIP-42 身份认证；普通事件和订阅受规则限制，私信 kind 4、13、14、1059 不受黑白名单限制；每行一条，格式为：公钥 | 用户名（用户名可选）。": "accessDescription", "读取始终经过它，再选延迟最低的两个；发布会优先发送给它并继续发送到其他在线上游。": "priorityDescription", "（已停用）": "disabledRelay"
};

const UI_ATTRIBUTE_KEYS = {
  "管理员用户名": "username", "密码": "password", "relay.example.com": "relayAddress", "Relay 地址": "relayAddress", "中继名称": "relayName", "描述": "description", "机主公钥 hex": "ownerPubkey", "联系方式，例如 mailto:you@example.com": "contact", "头像 URL": "iconUrl", "拖动排序": "dragHandle", "点击按延迟排序": "sortLatency", "npub1… 或 64 位 hex 公钥 | 用户名，每行一条": "userPubkey", "公钥（npub 或 hex）": "userPubkey", "用户名": "userName"
};

function localizationScript() {
  return `<script>(()=>{const translations=${JSON.stringify(UI_TRANSLATIONS)};const staticKeys=${JSON.stringify(UI_STATIC_KEYS)};const attributeKeys=${JSON.stringify(UI_ATTRIBUTE_KEYS)};const stored=(()=>{try{return localStorage.getItem('nostr-relay-locale')}catch{return null}})();const locale=stored==='en'||stored==='zh'?stored:(navigator.language||'').toLowerCase().startsWith('zh')?'zh':'en';const t=key=>(translations[locale]?.[key]??translations.zh[key]??key);window.t=t;window.currentLocale=()=>locale;const age=value=>{let m;if((m=/^(\\d+) 小时 (\\d+) 分钟$/.exec(value)))return t('hoursMinutes').replace('{h}',m[1]).replace('{m}',m[2]);if((m=/^(\\d+) 分 (\\d+) 秒$/.exec(value)))return t('minutesSeconds').replace('{m}',m[1]).replace('{s}',m[2]);if((m=/^(\\d+) 秒$/.exec(value)))return t('seconds').replace('{s}',m[1]);if(value==='刚刚')return t('justNow');return value};const translateValue=value=>{const key=staticKeys[value];if(key)return t(key);let m;if(value.startsWith('服务在线 · ')&&value.endsWith(' 个上游已连接'))return t('serviceOnline')+' · '+t('upstreamsConnected').replace('{n}',value.slice(7,-7));if(value.startsWith('读取于 '))return t('readAt').replace('{time}',value.slice(4));if(value.startsWith('已连接 '))return t('connected')+' '+age(value.slice(4));if(value.startsWith('等待重连 · 已重连 '))return t('waitingReconnect').replace('{n}',value.slice(11,-2));if(value.startsWith('当前 ')&&value.includes(' 个已认证且通过规则的用户；已认证公钥总数 ')){m=/^当前 (\\d+) 个已认证且通过规则的用户；已认证公钥总数 (\\d+)。$/.exec(value);if(m)return t('currentUsersHint').replace('{n}',m[1]).replace('{total}',m[2])}return value};let applying=false;const apply=()=>{if(applying)return;applying=true;document.documentElement.lang=locale==='zh'?'zh-CN':'en';document.title=document.querySelector('.login')?t('loginTitle'):location.pathname==='/admin'?t('adminTitle'):'Nostr Relay Proxy';const walk=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const nodes=[];while(walk.nextNode())nodes.push(walk.currentNode);for(const node of nodes){const raw=node.nodeValue;const value=raw.trim();if(!value)continue;const translated=translateValue(value);if(translated!==value){const offset=raw.indexOf(value);node.nodeValue=raw.slice(0,offset)+translated+raw.slice(offset+value.length)}}for(const element of document.querySelectorAll('[placeholder],[aria-label],[title]'))for(const attr of ['placeholder','aria-label','title']){const key=attributeKeys[element.getAttribute(attr)];if(key)element.setAttribute(attr,t(key))}const select=document.getElementById('language-select');if(select)select.value=locale;applying=false};apply();if(locale==='en'){let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;queueMicrotask(()=>{queued=false;apply()})}).observe(document.body,{childList:true,subtree:true,characterData:true})}document.getElementById('language-select')?.addEventListener('change',event=>{try{localStorage.setItem('nostr-relay-locale',event.target.value)}catch{}location.reload()})})()</script>`;
}

const LANGUAGE_CONTROL = `<select class="input mini language-select" id="language-select" aria-label="Language"><option value="zh">中文</option><option value="en">English</option></select>`;
...
function renderAdminHtml() {
  const mobileTableStyle = `<style>@media(min-width:901px){.relay-admin-grid{grid-template-columns:minmax(0,2fr) minmax(420px,1fr)}}.settings-panel .form-row{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));align-items:start;gap:10px}.settings-panel .form-row>h2,.settings-panel .form-row>p,.settings-panel .form-row>hr,.settings-panel .form-row>.muted,.settings-panel .form-row>select,.settings-panel .form-row>textarea,.settings-panel .form-row>.access-user-list,.settings-panel .form-row>#add-access-user,.settings-panel .form-row>#save-settings{grid-column:1/-1}.settings-panel .form-row>h2{margin-top:8px;padding-top:12px;border-top:1px solid #28445f}.settings-panel .form-row>h2:first-child{margin-top:0;padding-top:0;border-top:0}.settings-panel .form-row>.button.primary{width:100%}.access-user-row{display:grid;grid-template-columns:minmax(0,1.6fr) minmax(90px,1fr) auto;gap:6px}.drag-handle{display:inline-block;width:18px;margin-right:6px;color:#8ea2ba;font-size:15px;line-height:1;cursor:grab;user-select:none}.relay-panel .table tr[draggable=true]{cursor:grab}.relay-panel .table tr.dragging{opacity:.45}.relay-panel .table tr.relay-highlight{animation:relay-highlight 2.2s ease-out}.relay-panel .table tr[draggable=true]:active,.drag-handle:active{cursor:grabbing}@keyframes relay-highlight{0%,35%{background:#304b36;box-shadow:0 0 0 2px #8ee0b5}100%{background:transparent;box-shadow:none}}.relay-panel .table th:first-child,.relay-panel .table td:first-child{width:1%;min-width:0!important;white-space:nowrap}@media(max-width:720px){.settings-panel .form-row{display:flex!important;flex-direction:column!important;gap:8px}.settings-panel .form-row>*{width:100%}.access-user-row{grid-template-columns:1fr}.relay-panel,.relay-panel .table-wrap{min-width:0}.relay-panel .table-wrap{max-width:100%;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch}.relay-panel .table{display:table;width:max-content;min-width:100%}.relay-panel .table thead{display:table-header-group}.relay-panel .table tbody{display:table-row-group}.relay-panel .table tr{display:table-row;margin:0;padding:0;border:0;background:transparent}.relay-panel .table th,.relay-panel .table td{display:table-cell;width:1%;min-width:auto!important;white-space:nowrap!important;padding:8px 7px;border-bottom:1px solid #222b37;text-align:left;word-break:normal}.relay-panel .table td::before{display:none}}</style>`;
  ...
  const relayOrderScript = `<script>(()=>{const tbody=document.getElementById('relays');if(!tbody)return;let dragging=null;const notice=(message,ok=true)=>{const el=document.getElementById('notice');if(!el)return;el.textContent=message;el.style.display='block';el.style.color=ok?'#7ef0bd':'#ffb5c3';if(ok)setTimeout(()=>{if(el.textContent===message)el.style.display='none'},2200)};const save=async()=>{const urls=[...tbody.querySelectorAll('tr')].map(row=>row.dataset.url).filter(Boolean);if(urls.length<2)return;try{const response=await fetch('/admin/api/relays/reorder',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({urls})});if(!response.ok)throw new Error('保存排序失败');notice('上游 Relay 排序已保存')}catch(error){notice(error.message||'保存排序失败',false)}};const bind=()=>{for(const row of tbody.querySelectorAll('tr')){if(row.dataset.dragBound)continue;row.dataset.dragBound='1';row.draggable=true;row.addEventListener('dragstart',event=>{dragging=row;row.classList.add('dragging');event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',row.dataset.url||'')});row.addEventListener('dragend',()=>{row.classList.remove('dragging');dragging=null});row.addEventListener('dragover',event=>{event.preventDefault();if(!dragging||dragging===row)return;const rect=row.getBoundingClientRect();tbody.insertBefore(dragging,event.clientY<rect.top+rect.height/2?row:row.nextSibling)});row.addEventListener('drop',event=>{event.preventDefault();save()})}};new MutationObserver(bind).observe(tbody,{childList:true});bind()})()</script>`;
  const latencySortScript = `<script>(()=>{const tbody=document.getElementById('relays');const header=tbody?.closest('table')?.querySelector('thead th:nth-child(3)');if(!tbody||!header)return;let direction=0;const label=()=>window.t?.('latency')||'延迟';const reset=()=>{direction=0;header.textContent=label();header.title=window.t?.('sortLatency')||'点击按延迟排序'};const sort=()=>{direction=direction===1?-1:1;const rows=[...tbody.querySelectorAll('tr')];rows.sort((a,b)=>{const av=Number(a.dataset.latency),bv=Number(b.dataset.latency);const ai=Number.isFinite(av),bi=Number.isFinite(bv);if(ai!==bi)return ai?-1:1;return ai?direction*(av-bv):0});tbody.append(...rows);header.textContent=label()+(direction===1?' ↑':' ↓')};header.style.cursor='pointer';header.title=window.t?.('sortLatency')||'点击按延迟排序';header.addEventListener('click',sort);const original=window.refresh;if(typeof original==='function')window.refresh=async(...args)=>{reset();return original(...args)};reset()})()</script>`;
  const addRelayScript = `<script>(()=>{const input=document.getElementById('url');const button=document.getElementById('add');const tbody=document.getElementById('relays');if(!input||!button||!tbody)return;const normalize=value=>{const raw=String(value||'').trim();if(!raw)return '';const lower=raw.toLowerCase();if(lower.startsWith('wss://'))return 'wss://'+raw.slice(6);if(lower.startsWith('ws://'))return 'ws://'+raw.slice(5);return raw.includes('://')?raw:'wss://'+raw};const notice=(message,ok=true)=>{const el=document.getElementById('notice');if(!el)return;el.textContent=message;el.style.display='block';el.style.color=ok?'#7ef0bd':'#ffb5c3';if(ok)setTimeout(()=>{if(el.textContent===message)el.style.display='none'},2500)};const highlight=url=>{const row=[...tbody.querySelectorAll('tr')].find(item=>item.dataset.url===url);if(!row)return;tbody.querySelectorAll('.relay-highlight').forEach(item=>item.classList.remove('relay-highlight'));row.classList.add('relay-highlight');row.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(()=>row.classList.remove('relay-highlight'),2400)};button.onclick=async()=>{const url=normalize(input.value);if(!url)return;button.disabled=true;try{const response=await fetch('/admin/api/relays',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url})});if(response.status===401){location.href='/admin';return}const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'添加 Relay 失败');const existing=response.headers.get('x-relay-existing')==='1';input.value='';await window.refresh?.();setTimeout(()=>highlight(url),30);notice(existing?'Relay 已存在，已定位到该行':'Relay 已添加，已定位到该行')}catch(error){notice(error.message||'添加 Relay 失败',false)}finally{button.disabled=false}}})()</script>`;
  ...
  return ADMIN_HTML
    ...
    .replace('白名单和黑名单会要求客户端完成 NIP-42 身份认证；每行填入一个用户公钥（hex 或 npub）。', '白名单和黑名单会要求客户端完成 NIP-42 身份认证；普通事件和订阅受规则限制，私信 kind 4、13、14、1059 不受黑白名单限制；每行填入一个用户公钥（hex 或 npub）。')
    ...
    .replace('<aside class="panel section">', '<aside class="panel section settings-panel">')
    ...
    .replace("const tbody=document.getElementById('relays');tbody.replaceChildren(...s.relays.map(r=>{const tr=document.createElement('tr');", "const tbody=document.getElementById('relays');tbody.replaceChildren(...s.relays.map(r=>{const tr=document.createElement('tr');tr.dataset.url=r.url;tr.dataset.latency=String(r.connected&&Number.isFinite(r.latencyMs)?r.latencyMs:'');")
    .replace("td.textContent=v;if(i===1)td.className=r.connected?'badge on':'badge off';tr.append(td)", "td.textContent=v;if(i===0){const handle=document.createElement('span');handle.className='drag-handle';handle.textContent='⠿';handle.title='拖动排序';td.prepend(handle)}if(i===1){const badge=document.createElement('span');badge.className=r.connected?'badge on':'badge off';badge.textContent=v;td.textContent='';td.append(badge)}tr.append(td)")
    .replace("card('在线用户',s.clients),", "card(s.settings?.accessMode==='all'?'在线连接':'有效用户',s.clients),card('已认证用户',s.authenticatedUsers??0),")
    .replace("new Option('不设置（仅按延迟）','')", "new Option(window.t?.('noPriority')||'不设置（仅按延迟）','')")
    .replace("option.textContent+='（已停用）';", "option.textContent+=(window.t?.('disabledRelay')||'（已停用）');")
    .replace("if(confirm('删除 '+r.url+'？'))", "if(confirm((window.t?.('confirmDelete')||'删除 {url}？').replace('{url}',r.url)))")
    .replace(';refresh();</script>', ';setTimeout(refresh,0);</script>')
    ...
    .replace('</body>', `${activeUsersScript}${activeUsersNamesScript}${accessUsersFormScript}${namedAccessSettingsScript}${relayOrderScript}${latencySortScript}${addRelayScript}${safeAdminRefreshScript}${adminVisibilityScript}${localizationScript()}</body>`);
}
...
function renderHomeHtml() {
  ...
  ).replace('<a class="button" href="/admin">管理后台</a>', `<div class="nav-actions">${LANGUAGE_CONTROL}<a class="button" href="/admin">管理后台</a></div>`)
    .replace('<main class="shell">', '<main class="shell home-shell">')
    .replace("const ago=n=>{if(!n)return ui('noValue');let s=Math.max(0,Math.floor((Date.now()-n)/1000));let h=Math.floor(s/3600),m=Math.floor(s%3600/60);return h?ui('hoursMinutes').replace('{h}',h).replace('{m}',m):m?ui('minutesSeconds').replace('{m}',m).replace('{s}',s%60):ui('justNow')};", "const ago=n=>{if(!n)return ui('noValue');let seconds=Math.max(0,Math.floor((Date.now()-n)/1000));const days=Math.floor(seconds/86400);seconds%=86400;const hours=Math.floor(seconds/3600);seconds%=3600;const minutes=Math.floor(seconds/60);if(days){const en=window.currentLocale?.()==='en';return en?days+(hours?'d '+hours+'h':'d'):days+'天'+(hours?' '+hours+'小时':'')}if(hours)return ui('hoursMinutes').replace('{h}',hours).replace('{m}',minutes);if(minutes)return ui('minutesSeconds').replace('{m}',minutes).replace('{s}',seconds%60);return ui('justNow')};")
    .replace('</body>', `${localizationScript()}</body>`);
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/") {
      ...
      return new Response(renderHomeHtml(), {
        headers: {
          ...
          // Never keep an old inline script after a recovery or UI fix.
    ...
    if (url.pathname === "/admin" || url.pathname.startsWith("/admin/")) {
      ...
      if (request.method === "POST" && url.pathname === "/admin/api/relays/reorder") {
        const response = await durableFetch(env, new Request("https://relay/relays/reorder", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: await request.text()
        }));
...
export class RelayHub {
  ...
  async handleFetch(request) {
    ...
    if (url.pathname === "/relays" && request.method === "POST") {
      ...
      if (this.relays.some(r => r.url === relay))
        return json(this.relays, 200, { "x-relay-existing": "1" });
      if (!this.relays.some(r => r.url === relay)) {
      ...
      return json(this.relays, 200, { "x-relay-existing": "0" });
    }

    if (url.pathname === "/relays/reorder" && request.method === "POST") {
      const body = await request.json().catch(() => ({}));
      const urls = Array.isArray(body.urls) ? body.urls.map(url => normalizeRelayUrl(url)) : [];
      const current = new Map(this.relays.map(relay => [relay.url, relay]));
      if (urls.length !== this.relays.length || new Set(urls).size !== this.relays.length || urls.some(url => !current.has(url)))
        return json({ error: "invalid relay order" }, 400);
      this.relays = urls.map(url => current.get(url));
      await this.persistRelays();
      return json(this.relays);
    }
  ...
  async onClientMessage(clientId, raw) {
    ...
    const privateMessage = type === "EVENT"
      ? PRIVATE_MESSAGE_KINDS.has(Number(msg[1]?.kind))
      : (type === "REQ" || type === "COUNT") && this.isPrivateMessageSubscription(msg.slice(2));
    if (type !== "CLOSE" && !privateMessage) {
    ...
    if (type === "COUNT") {
      ...
      const open = this.subscriptionUpstreams(filters);
...
const ui=k=>window.t?.(k)||k;const fill=(k,v)=>ui(k).replace('{n}',String(v));const wsUrl=location.origin.replace(/^http/,'ws')+'/';document.getElementById('copy').onclick=async()=>{await navigator.clipboard.writeText(wsUrl);document.getElementById('copy').textContent=ui('copied')+' '+wsUrl;setTimeout(()=>document.getElementById('copy').textContent=ui('copyUrl'),1800)};
const ago=n=>{if(!n)return ui('noValue');let s=Math.max(0,Math.floor((Date.now()-n)/1000));let h=Math.floor(s/3600),m=Math.floor(s%3600/60);return h?ui('hoursMinutes').replace('{h}',h).replace('{m}',m):m?ui('minutesSeconds').replace('{m}',m).replace('{s}',s%60):ui('justNow')};
function metric(label,value){let el=document.createElement('div');el.className='metric panel';el.innerHTML='<div class="value"></div><div class="label"></div>';el.children[0].textContent=value;el.children[1].textContent=ui(label);return el}
let refreshTimer;function schedule(seconds=60){clearTimeout(refreshTimer);refreshTimer=setTimeout(refresh,Math.max(60,Number(seconds)||60)*1000)}function renderUnavailable(){clearTimeout(refreshTimer);document.getElementById('dot').className='dot';document.getElementById('summary').textContent=ui('unavailable');document.getElementById('updated').textContent='';document.getElementById('metrics').replaceChildren(metric('effectiveUsers',0),metric('clientSubscriptions',0),metric('upstreamSubscriptions',0),metric('availableRelays','0 / 0'),metric('serviceUptime',ui('noValue')));const message=document.createElement('div');message.className='muted';message.textContent=ui('unavailable');document.getElementById('relays').replaceChildren(message)}async function refresh(){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),4000);try{const r=await fetch('/status',{cache:'no-store',signal:controller.signal});if(!r.ok)throw Error('status unavailable');const s=await r.json();if(s.degraded){renderUnavailable();return}const active=(s.relays||[]).filter(x=>x.connected).length;const dot=document.getElementById('dot');dot.className='dot '+(s.online?'on':'');document.getElementById('summary').textContent=s.online?ui('serviceOnline')+' · '+fill('upstreamsConnected',active):ui('waitingUpstream');document.getElementById('updated').textContent=ui('readAt').replace('{time}',new Date().toLocaleTimeString());const metrics=document.getElementById('metrics');metrics.replaceChildren(metric('effectiveUsers',s.clients||0),metric('clientSubscriptions',s.clientSubscriptions??s.subscriptions??0),metric('upstreamSubscriptions',s.upstreamSubscriptions??0),metric('availableRelays',active+' / '+(s.relays||[]).length),metric('serviceUptime',ago(s.stats?.startedAt)));const list=document.getElementById('relays');list.replaceChildren(...(s.relays||[]).map(x=>{let row=document.createElement('div');row.className='relay';let left=document.createElement('div');let name=document.createElement('div');name.className='relay-name';name.textContent=x.url;let meta=document.createElement('div');meta.className='muted';meta.textContent=x.connected?ui('connected')+' '+ago(x.lastConnectedAt):x.enabled?ui('waitingReconnect').replace('{n}',x.reconnects):ui('disabled');left.append(name,meta);let badge=document.createElement('span');badge.className='badge '+(x.connected?'on':'off');badge.textContent=x.connected?ui('online'):ui('offline');row.append(left,badge);return row}));schedule(s.settings?.statusRefreshSeconds)}catch{renderUnavailable()}finally{clearTimeout(timeout)}}
setTimeout(refresh,0);</script></body></html>`;

const LOGIN_HTML = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>登录 · Nostr Relay Proxy</title><style>${BASE_STYLE}</style></head><body><main class="shell"><div class="panel login"><div class="brand"><span class="mark">ϟ</span>Nostr Relay Proxy</div><h1>欢迎回来</h1><p class="muted">登录后管理上游 Relay 与实时网络状态。</p><form id="form"><input class="input" id="username" autocomplete="username" placeholder="管理员用户名" required><input class="input" id="password" type="password" autocomplete="current-password" placeholder="密码" required><button class="button primary" id="submit">安全登录</button><div class="notice" id="notice">__ERROR__</div></form></div></main><script>document.getElementById('form').onsubmit=async e=>{e.preventDefault();const ui=k=>window.t?.(k)||k;const b=document.getElementById('submit');b.disabled=true;b.textContent=ui('loggingIn');const user=document.getElementById('username').value;const pass=document.getElementById('password').value;const r=await fetch('/admin/api/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:user,password:pass})});if(r.ok){location.href='/admin';return}const d=await r.json().catch(()=>({error:ui('loginFailed')}));const n=document.getElementById('notice');n.textContent=d.error||ui('loginFailed');n.style.display='block';b.disabled=false;b.textContent=ui('safeLogin')}</script></body></html>`;

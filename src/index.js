  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/admin" || url.pathname.startsWith("/admin/")) {
      if (url.pathname === "/admin/api/login" && request.method === "POST") {
        ...
        return json({ ok: true }, 200, { "set-cookie": `relay_admin=${encodeURIComponent(session)}; Path=/admin; HttpOnly; Secure; SameSite=Strict; Max-Age=28800` });
      }
      if (url.pathname === "/admin/api/logout" && request.method === "POST")
        return json({ ok: true }, 200, { "set-cookie": "relay_admin=; Path=/admin; HttpOnly; Secure; SameSite=Strict; Max-Age=0" });
...
export class RelayHub {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/public-status") return json({
      ...
      subscriptions: [...this.clients.values()].reduce((n, c) => n + c.subscriptions.size, 0),
      stats: this.stats,
      relays: this.relays.map(({ url, enabled, connected, reconnects, lastConnectedAt }) => ({ url, enabled, connected, reconnects, lastConnectedAt }))
...
const BASE_STYLE = `
:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;background:#07111f;color:#edf5ff}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% 0,#193e6c 0,transparent 33rem),radial-gradient(circle at 90% 5%,#123c38 0,transparent 28rem),#07111f;min-height:100vh}a{color:inherit}.shell{max-width:1120px;margin:auto;padding:32px 20px 56px}.brand{display:flex;align-items:center;gap:12px;font-weight:750;letter-spacing:-.03em}.mark{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(135deg,#6ee7d6,#5797ff);color:#07111f;font-size:21px}.nav{display:flex;justify-content:space-between;align-items:center;gap:16px}.eyebrow{color:#7de5d6;text-transform:uppercase;letter-spacing:.13em;font-size:12px;font-weight:750}.hero{padding:82px 0 48px;max-width:760px}.hero h1{font-size:clamp(38px,7vw,68px);letter-spacing:-.065em;line-height:1.02;margin:16px 0}.lead{font-size:18px;color:#b4c4d8;line-height:1.65;max-width:650px}.actions{display:flex;gap:12px;flex-wrap:wrap;margin-top:28px}.button{border:1px solid #45668e;border-radius:12px;padding:11px 15px;background:#10233c;color:#f4f8ff;font-weight:700;cursor:pointer;text-decoration:none;font:inherit}.button.primary{background:linear-gradient(135deg,#69e1d0,#6094ff);color:#07111f;border:0}.button.danger{background:#3a1823;border-color:#733448}.button:hover{filter:brightness(1.12)}.panel{background:linear-gradient(135deg,#10233cbb,#0d1b2dcc);border:1px solid #26486e;border-radius:20px;box-shadow:0 22px 60px #0004}.status-bar{display:flex;gap:10px;align-items:center;color:#b9cbe0;font-size:14px}.dot{width:9px;height:9px;border-radius:50%;background:#fa7188;box-shadow:0 0 0 5px #fa718822}.dot.on{background:#66e3a3;box-shadow:0 0 0 5px #66e3a322}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.metric{padding:18px}.metric .value{font-weight:780;font-size:30px;letter-spacing:-.05em}.metric .label,.muted{font-size:13px;color:#a9bdd5}.section{margin-top:22px;padding:24px}.section h2{font-size:20px;letter-spacing:-.03em;margin:0 0 6px}.relay-list{display:grid;gap:10px;margin-top:18px}.relay{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:15px;border-radius:13px;background:#07182b99;border:1px solid #254667}.relay-name{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:14px;word-break:break-all}.badge{font-size:12px;padding:5px 9px;border-radius:999px;background:#26374b;color:#c7d5e7;white-space:nowrap}.badge.on{background:#103e32;color:#7ef0bd}.badge.off{background:#42212b;color:#ffb2c1}.copy{font:inherit;border:0;background:transparent;color:#90b9ff;cursor:pointer;padding:0}.admin-grid{display:grid;grid-template-columns:1.1fr .9fr;gap:18px}.form-row{display:flex;gap:10px;margin-top:18px}.form-row input{flex:1;min-width:0}.input{border:1px solid #345575;border-radius:12px;padding:12px 14px;background:#081727;color:#eef7ff;font:inherit}.table-wrap{overflow:auto}.table{width:100%;border-collapse:collapse;margin-top:14px}.table th,.table td{padding:14px 9px;text-align:left;border-bottom:1px solid #28445f;font-size:14px}.table th{font-size:12px;color:#9db5d0;text-transform:uppercase;letter-spacing:.08em}.table td:first-child{word-break:break-all;min-width:240px}.mini{padding:7px 10px;border-radius:9px;font-size:12px}.notice{display:none;margin-top:12px;color:#ffb5c3;font-size:14px}.login{max-width:430px;margin:10vh auto;padding:30px}.login h1{letter-spacing:-.04em;margin:18px 0 8px}.login form{display:grid;gap:12px;margin-top:24px}.login .button{width:100%}@media(max-width:720px){.shell{padding:22px 15px}.hero{padding:58px 0 35px}.grid{grid-template-columns:repeat(2,1fr)}.admin-grid{grid-template-columns:1fr}.relay{align-items:flex-start;flex-direction:column}.form-row{flex-direction:column}.form-row .button{width:100%}}
`;

const HOME_HTML = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nostr Relay Proxy</title><style>${BASE_STYLE}</style></head><body><main class="shell"><nav class="nav"><div class="brand"><span class="mark">ϟ</span>Nostr Relay Proxy</div><a class="button" href="/admin">管理后台</a></nav><section class="hero"><div class="eyebrow">Multi-relay gateway</div><h1>一个更可靠的<br> Nostr 连接入口。</h1><p class="lead">聚合多个上游 Relay，为你的客户端提供统一、稳定的 WebSocket 连接。状态实时更新，无需刷新页面。</p><div class="actions"><button class="button primary" id="copy">复制连接地址</button><a class="button" href="/status" target="_blank">查看状态 JSON</a></div></section><section class="panel section"><div class="status-bar"><span id="dot" class="dot"></span><span id="summary">正在获取网络状态…</span><span class="muted" id="updated"></span></div><div class="grid" id="metrics" style="margin-top:20px"></div></section><section class="panel section"><h2>上游 Relay</h2><p class="muted">已连接的上游会自动同步订阅；离线节点会采用退避策略重连。</p><div class="relay-list" id="relays"><div class="muted">加载中…</div></div></section></main><script>

...
- 独立的后台登录页与 8 小时安全会话
- 上游 Relay 增删、启停
- 后台上游 Relay 列表支持拖动排序，顺序会保存并同步到前台展示；读取路由仍按优先中继和延迟策略选择
...
白名单和黑名单均使用 NIP-42 身份认证验证客户端签名，不能只凭客户端自报公钥。普通事件和订阅受黑白名单限制；私信 kind `4`、`13`、`14`、`1059` 不受黑白名单限制。后台名单每行格式为“公钥 | 用户名”，用户名仅用于后台识别；公钥可填 64 位十六进制或 `npub1...`，保存时会去重并转换为十六进制。启用这两种模式后，不支持 NIP-42 的客户端仍可使用私信功能，但不能使用普通事件和订阅。
...
- Durable Object 暂时过载时，Worker 层会熔断 10 秒，避免继续冲击同一个 DO；首页状态返回带 `degraded: true` 的离线快照，NIP-11 回退到环境变量/默认资料，健康页和静态页面仍可访问。WebSocket 与后台写操作会返回 503，并要求稍后重试。
...
- 空闲保护：没有客户端连接时不会启动上游 Relay，也不会保留重连 Alarm；第一个客户端连接后才启动上游，最后一个客户端断开后会停止上游并清理 Alarm。首次连接可能需要等待上游完成连接。
- 有客户端连接时，重连时间会登记到 Durable Object Alarm；即使对象因短暂空闲被回收，也能按保存的退避时间恢复失联中继。没有客户端时会主动清理 Alarm，避免后台持续唤醒。
...
- `04`：透明转发 NIP-04 kind `4` 加密私信；私信订阅会查询所有在线上游并按事件 ID 去重。
- `11`：对客户端标准的 NIP-11 请求返回后台配置的中继名称、描述、机主公钥、头像、联系方式与支持的 NIPs。
- `17`：透明转发 NIP-17 私信流程涉及的 kind `13`、`14` 和 `1059` 事件。
...
- `59`：透明转发 NIP-59 Gift Wrap 私信事件；私信订阅会查询所有在线上游并按事件 ID 去重。
- `77`：支持 Negentropy 会话消息的上游路由，当前会选择一个在线上游 Relay 透传 `NEG-OPEN / NEG-MSG / NEG-CLOSE`。

私信事件不会被代理解密或修改。NIP-04 kind `4` 以及 NIP-17/NIP-59 常见的 kind `13`、`14`、`1059` 会透明转发；客户端针对这些 kind 的订阅会查询所有当前在线的上游 Relay，并按事件 ID 去重，避免某个 Relay 没有同步到私信时收不到消息。

`02 / 09 / 28 / 40` 是事件格式或事件语义。代理会透明转发这些事件，但它们不是由本代理独立实现和存储的 Relay 功能，因此不会写进 `supported_nips`。NIP-70 的受保护事件需要 NIP-42 认证；本代理会按规范拒绝它们，也不会宣称支持 NIP-70。

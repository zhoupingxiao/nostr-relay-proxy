...
const UI_TRANSLATIONS = {
  zh: {
    ...
    authenticatedUsers: "已认证用户", onlineConnections: "在线连接", availableUpstream: "可用上游", uploadedTraffic: "上传流量", downloadedTraffic: "下载流量", clientEvents: "客户端事件", dragHandle: "拖动排序", hintStorage: "提示：上游列表、中继资料和优先中继都保存在 Durable Object；Cloudflare 中的变量仅用于首次默认值。",
  ...
  en: {
    ...
    authenticatedUsers: "Authenticated users", onlineConnections: "Online connections", availableUpstream: "Available upstream", uploadedTraffic: "Uploaded traffic", downloadedTraffic: "Downloaded traffic", clientEvents: "Client events", dragHandle: "Drag to reorder", hintStorage: "Hint: upstream Relays, relay profile and priority Relay are stored in the Durable Object; Cloudflare variables are only used as first-run defaults.",
...
function renderAdminHtml() {
  ...
  return ADMIN_HTML
    ...
    .replace("const tbody=document.getElementById('relays');tbody.replaceChildren(...s.relays.map(r=>{const tr=document.createElement('tr');", "const tbody=document.getElementById('relays');tbody.replaceChildren(...s.relays.map(r=>{const tr=document.createElement('tr');tr.dataset.url=r.url;")
    ...
    .replace('</body>', `${activeUsersScript}${activeUsersNamesScript}${accessUsersFormScript}${namedAccessSettingsScript}${relayOrderScript}${addRelayScript}${safeAdminRefreshScript}${adminVisibilityScript}${localizationScript()}</body>`);
}
